...
(()=>{
  ...
  let coreRecords=[],mapped=false,categoryNodes=[],selectedCategory='',panel=null;
  ...
  function positionCoreHost(node,host){
    const rect=node.getBoundingClientRect();
    const center=rect.left+rect.width/2;
    const mobile=window.matchMedia('(max-width:760px)').matches;
    const need=mobile?156:214;
    const roomRight=window.innerWidth-rect.right;
    const roomLeft=rect.left;
    const side=roomRight>=need||roomRight>=roomLeft?'right':'left';
    host.dataset.side=side;
    host.style.left=side==='right'?'calc(100% + var(--core-gap))':'auto';
    host.style.right=side==='left'?'calc(100% + var(--core-gap))':'auto';
    host.style.top='50%';
    host.style.transform='translateY(-50%)';
    host.style.minWidth=mobile?'136px':'168px';
    host.style.justifyItems=side==='right'?'start':'end';
    host.style.textAlign=side==='right'?'left':'right';
    if(mobile&&center<window.innerWidth*.25)host.dataset.side='right';
    if(mobile&&center>window.innerWidth*.75)host.dataset.side='left';
  }
  ...
  function ensureCoreHost(node){
    ...
    if(!host){
      ...
      host.style.cssText='position:absolute;display:grid;gap:8px;z-index:5;pointer-events:auto';
      node.style.overflow='visible';node.appendChild(host);
    }
    positionCoreHost(node,host);return host;
  }
  ...
  function renderCoreChildren(detail){
    ...
    CATEGORY_ORDER.forEach((category,index)=>{
      const node=categoryNodes[index];if(!node)return;
      const records=coreRecords.filter(record=>record.category===category);
      const host=ensureCoreHost(node);host.innerHTML='';
      records.forEach(record=>{
        ...
        card.className='esafe-core-card';
        card.dataset.nodeId=`esafe-core-${record.id}`;card.dataset.esafeParentCategory=category;card.dataset.esafeCore='true';
      ...
      host.style.display=records.length?'grid':'none';positionCoreHost(node,host);
    });
  }

  function ensurePanel(){
    if(panel)return panel;
    panel=document.createElement('aside');panel.id='esafeRecordPreview';panel.setAttribute('aria-hidden','true');
    ...
    document.body.appendChild(panel);panel.querySelector('#esafePreviewClose')?.addEventListener('click',closePanel);return panel;
  }

  function closePanel(){if(!panel)return;selectedCategory='';panel.setAttribute('aria-hidden','true');panel.style.opacity='0';panel.style.pointerEvents='none';panel.style.transform='translate(-50%,18px)'}

  function openCategory(category){
    selectedCategory=category;const host=ensurePanel();const state=categoryState.get(category)||{visible:0,total:0};const preview=categoryPreviews.get(category)||[];
    const title=host.querySelector('#esafePreviewTitle'),meta=host.querySelector('#esafePreviewMeta'),list=host.querySelector('#esafePreviewList');
    if(title)title.textContent=category;if(meta)meta.textContent=`${state.visible} visible now · ${state.total} total records · latest 3 shown`;
    if(list)list.innerHTML=preview.length?preview.map(record=>`<a href="${record.url||'#'}" target="_blank" rel="noopener" style="display:grid;grid-template-columns:72px 1fr auto;gap:8px;align-items:center;text-decoration:none;color:inherit;border:1px solid rgba(164,193,227,.16);border-radius:11px;padding:8px;background:rgba(255,255,255,.025)"><span style="font-size:9px;color:#9bb0c8">${record.date||'—'}</span><span style="min-width:0"><strong style="display:block;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${String(record.title||record.id)}</strong><small style="color:#9bb0c8">${record.core?'CORE PILOT · ':''}${record.fileCount||0} file${record.fileCount===1?'':'s'}</small></span><span style="font-size:16px">›</span></a>`).join(''):'<div style="padding:12px;border:1px dashed rgba(164,193,227,.18);border-radius:11px;color:#9bb0c8">No records are available in this category at the current timeline position.</div>';
    host.setAttribute('aria-hidden','false');host.style.opacity='1';host.style.pointerEvents='auto';host.style.transform='translate(-50%,0)';
  }

  function mapCanonicalNodes(){
    if(mapped)return true;const all=Array.from(document.querySelectorAll('[data-node-id]'));if(!all.length)return false;
    const project=all.find(node=>node.getAttribute('data-node-id')==='proj')||all[0];if(project){setNodeText(project,'e-SAFE Catania Real Pilot','Active Project World');project.dataset.esafeProject='true'}
    const candidates=all.filter(node=>node!==project).slice(0,CATEGORY_ORDER.length);if(candidates.length<CATEGORY_ORDER.length)return false;categoryNodes=candidates;
    CATEGORY_ORDER.forEach((category,index)=>{const state=categoryState.get(category),node=candidates[index];setNodeText(node,category,`${state.visible} / ${state.total} records`);node.dataset.esafeCategory=category;node.querySelector('button')?.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();openCategory(category)},true)});
    mapped=true;document.documentElement.dataset.esafeGraphHydrated='true';return true;
  }
  ...
  function render(detail){
    ...
    CATEGORY_ORDER.forEach((category,index)=>{const state=categoryState.get(category)||{visible:0,total:0},node=categoryNodes[index];setNodeText(node,category,`${state.visible} / ${state.total} records`);const future=state.total>0&&state.visible===0;node.classList.toggle('nexus-time-future',future);node.style.opacity=future?'0.34':'1'});
    renderCoreChildren(detail||{});if(selectedCategory)openCategory(selectedCategory);
  }

  const refreshCorePositions=()=>categoryNodes.forEach(node=>{const host=node?.querySelector(':scope > .esafe-core-children');if(host)positionCoreHost(node,host)});
  window.addEventListener('resize',refreshCorePositions,{passive:true});
  window.addEventListener('orientationchange',()=>window.setTimeout(refreshCorePositions,120));
  const scan=()=>{if(mapCanonicalNodes())render(window.__NEXUS_PROJECT_TIME__||{});else window.setTimeout(scan,180)};
  window.addEventListener('nexus:project-time-change',event=>render(event.detail||{}));window.addEventListener('DOMContentLoaded',()=>window.setTimeout(scan,120));
})();
