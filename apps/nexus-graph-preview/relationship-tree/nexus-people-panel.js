// NEXUS_PEOPLE_PANEL_V7_PERSON_CARD_V47_DOCK_AND_TIMELINE_VIEW
(()=>{
  ...
  const installDockFit=()=>{
    ...
    style.textContent=`
      ...
      @media(max-width:760px){
        ...
        .nexus-shell-panel,.nexus-project-switcher{bottom:calc(var(--nexus-bottom-dock-h) + env(safe-area-inset-bottom,0px) + 10px)!important;max-height:calc(100dvh - var(--nexus-top-rail-h,76px) - var(--nexus-bottom-dock-h) - env(safe-area-inset-bottom,0px) - 18px)!important;}
        .nexus-time-panel:not(.timeline-bar){max-height:calc(100dvh - var(--nexus-top-rail-h,76px) - var(--nexus-bottom-dock-h) - env(safe-area-inset-bottom,0px) - 12px)!important;overflow:auto!important;-webkit-overflow-scrolling:touch!important;}
  ...
  const findBottomDock=()=>{
    ...
    if(!root)return null;
    const controls=Array.from(root.querySelectorAll('button,a,[role="button"]')).filter(el=>['tasks','project','people','docs','tools','system'].includes(norm(el.textContent||el.getAttribute('aria-label')||'')));
    ...
    for(const control of controls){
      ...
      for(let depth=0;node&&depth<8;depth+=1,node=node.parentElement){
        ...
        const hitCount=['tasks','project','people','docs','tools','system'].filter(label=>text.includes(label)).length;
        const rect=node.getBoundingClientRect();
        const nearBottom=rect.bottom>viewportH-230;
        const looksDock=hitCount>=4&&rect.width>240&&rect.height>=54&&rect.height<=200&&nearBottom;
  ...
  const fitBottomDock=()=>{
    ...
    if(!dock.dataset.nexusInitialScrollFixed){dock.scrollLeft=0;dock.dataset.nexusInitialScrollFixed='true'}
    const root=document.getElementById('root');
    if(root)root.style.paddingBottom='calc(var(--nexus-bottom-dock-h) + env(safe-area-inset-bottom,0px))';
  };

  const scheduleDockFit=()=>{fitBottomDock();requestAnimationFrame(fitBottomDock);setTimeout(fitBottomDock,120);setTimeout(fitBottomDock,450);setTimeout(fitBottomDock,1100)};

  const installTimelineViewSwitcher=()=>{
    const panel=document.getElementById('nexusTimelinePanel');
    if(!panel||panel.dataset.timelineViewSwitcher==='true')return false;
    panel.dataset.timelineViewSwitcher='true';
    const getMode=()=>{try{return localStorage.getItem('nexus.timeline.viewMode')==='bar'?'bar':'card'}catch{return'card'}};
    const saveMode=mode=>{try{localStorage.setItem('nexus.timeline.viewMode',mode)}catch{}};
    const switcher=document.createElement('div');
    switcher.className='nexus-time-view-switch';
    switcher.setAttribute('role','group');
    switcher.setAttribute('aria-label','Timeline display mode');
    switcher.innerHTML='<button type="button" data-timeline-view="card">CARD</button><button type="button" data-timeline-view="bar">BAR</button>';
    panel.prepend(switcher);
    const setMode=mode=>{
      const next=mode==='bar'?'bar':'card';
      panel.classList.toggle('timeline-bar',next==='bar');
      panel.classList.toggle('timeline-card',next==='card');
      panel.dataset.timelineView=next;
      panel.querySelectorAll('[data-timeline-view]').forEach(button=>{
        const active=button.dataset.timelineView===next;
    ...
    switcher.addEventListener('click',event=>{
      ...
      setMode(button.dataset.timelineView);
    });
    setMode(getMode());
    return true;
  };

  const scheduleTimelineSwitcher=()=>{
    if(installTimelineViewSwitcher())return;
    setTimeout(installTimelineViewSwitcher,80);
    setTimeout(installTimelineViewSwitcher,350);
    setTimeout(installTimelineViewSwitcher,1000);
  };

  let kamilBridge={id:'p-kamil',label:'Kamil Karaszewski',sublabel:'Technical Advisor · Welding & Fabrication',cardUrl:CARD_URL,dataUrl:FETCHER_URL,treeUrl:TREE_URL,bridge:true};
  fetch(DATA_URL,{cache:'no-store'}).then(response=>response.ok?response.json():null).then(data=>{
    if(!data)return;
    kamilBridge={...kamilBridge,id:data.graph?.nodeId||kamilBridge.id,label:data.person?.displayName||data.graph?.label||kamilBridge.label,sublabel:data.graph?.sublabel||[data.person?.role,data.person?.trade].filter(Boolean).join(' · ')||kamilBridge.sublabel,cardUrl:data.links?.personCard||kamilBridge.cardUrl,dataUrl:data.links?.dataFetcher||kamilBridge.dataUrl,treeUrl:data.links?.relationshipTree||kamilBridge.treeUrl};
    window.dispatchEvent(new CustomEvent('nexus:person-data-bridge-ready',{detail:kamilBridge}));
  }).catch(()=>{});

  window.addEventListener('DOMContentLoaded',()=>{
    installDockFit();scheduleDockFit();scheduleTimelineSwitcher();
    ...
    const observer=new MutationObserver(()=>{scheduleDockFit();scheduleTimelineSwitcher()});
    ...
    const readPeople=()=>{
      ...
      document.querySelectorAll('[data-node-id]').forEach(node=>{
        ...
        const typeSpan=spans.find(span=>norm(span.textContent)==='person');if(!typeSpan)return;
        const candidates=spans.map(span=>(span.textContent||'').trim()).filter(Boolean).filter(text=>norm(text)!=='person'&&!norm(text).startsWith('received '));if(!candidates.length)return;
        const label=candidates[0];const sublabel=candidates[1]||'Project person';const isKamil=/\bkamil\b/i.test(label);
    ...
    const focusNode=person=>{
      ...
      if(person.button){person.button.click();person.node.scrollIntoView?.({behavior:'smooth',block:'center',inline:'center'});window.dispatchEvent(new CustomEvent('nexus:person-focus',{detail:{nodeId:person.id,label:person.label}}));return;}
    ...
    const render=()=>{
      ...
      const people=readPeople();if(!list)return;
      ...
      people.forEach(person=>{
        const row=document.createElement('div');row.className=`nexus-person-row${person.bridge?' bridge':''}`;
        const safeCard=person.cardUrl||CARD_URL;const safeData=person.dataUrl||FETCHER_URL;
        ...
        row.querySelector('[data-person-focus]')?.addEventListener('click',()=>focusNode(person));list.appendChild(row);
      });
    };
    ...
    const open=()=>{document.querySelectorAll('.nexus-shell-panel.open').forEach(item=>item.classList.remove('open'));document.getElementById('nexusProjectSwitcher')?.classList.remove('open');document.getElementById('nexusTimelinePanel')?.classList.remove('open');render();panel.classList.add('open');panel.setAttribute('aria-hidden','false');scrim?.classList.add('open');scheduleDockFit();};
