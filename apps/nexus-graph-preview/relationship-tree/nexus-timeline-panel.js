// NEXUS_PROJECT_TIME_KNOB_CONSOLE_20260817
(()=>{
  ...
  function injectStyle(){
    if($('nexusPTimeKnobConsoleStyle'))return;
    const style=document.createElement('style');
    style.id='nexusPTimeKnobConsoleStyle';
    style.textContent=`
      ...
      .nexus-ptime-left{display:grid;grid-template-rows:16px 24px 15px;gap:3px;min-width:0}
      .nexus-ptime-heading{display:none!important}.nexus-ptime-heading-row{display:none!important}.nexus-ptime-source{display:none!important}
      .nexus-ptime-ticker{position:relative;height:16px;border:1px solid rgba(151,193,232,.2);border-radius:5px;background:linear-gradient(180deg,rgba(3,10,19,.78),rgba(13,24,39,.76));overflow:hidden;display:flex;align-items:center;white-space:nowrap;box-shadow:inset 0 0 0 1px rgba(0,0,0,.28);-webkit-mask-image:linear-gradient(90deg,transparent 0,#000 9%,#000 91%,transparent 100%);mask-image:linear-gradient(90deg,transparent 0,#000 9%,#000 91%,transparent 100%)}
      .nexus-ptime-ticker-text{position:absolute;left:0;display:inline-flex;gap:16px;align-items:center;width:max-content;min-width:max-content;will-change:transform;font-size:6.3px;line-height:1;font-weight:900;letter-spacing:.11em;text-transform:uppercase;animation:nexusPTimeTicker 24s linear infinite;color:#a8bbcf}.nexus-ptime-ticker b{color:#7fd0ff;font-weight:950}.nexus-ptime-ticker strong{color:#eef6ff;font-weight:950}.nexus-ptime-ticker em{color:#ffd166;font-style:normal}.nexus-ptime-ticker span{color:#a8bbcf}@keyframes nexusPTimeTicker{from{transform:translateX(100%)}to{transform:translateX(-100%)}}
      ...
      .nexus-ptime-transport button.led-blue.flash::after{background:#69d8ff;box-shadow:0 0 8px rgba(105,216,255,.7),inset 0 0 2px rgba(255,255,255,.55)}.nexus-ptime-transport button.active{border-color:rgba(111,231,255,.72)!important;background:linear-gradient(180deg,rgba(22,58,88,.95),rgba(13,39,64,.96))!important;color:#fff!important}
      ...
      .nexus-ptime-knob-zone{position:relative;display:grid;grid-template-rows:1fr 14px;align-items:center;justify-items:center;min-width:0;height:58px}
      .nexus-ptime-knob{position:relative;width:43px;height:43px;border-radius:50%;border:1px solid rgba(151,193,232,.25);background:radial-gradient(circle at 35% 28%,rgba(238,246,255,.16),rgba(23,38,57,.9) 39%,rgba(4,12,23,.96) 70%);box-shadow:inset -5px -7px 12px rgba(0,0,0,.55),inset 4px 4px 8px rgba(255,255,255,.06),0 7px 18px rgba(0,0,0,.35);padding:0;color:#eef6ff;cursor:pointer;touch-action:manipulation}
      .nexus-ptime-knob::before{content:'';position:absolute;left:50%;top:6px;width:3px;height:13px;border-radius:3px;background:#7fd0ff;transform:translateX(-50%);box-shadow:0 0 7px rgba(127,208,255,.72)}.nexus-ptime-knob::after{content:'';position:absolute;inset:12px;border-radius:50%;border:1px solid rgba(151,193,232,.18)}
      .nexus-ptime-knob-label{font-size:6px;font-weight:950;letter-spacing:.12em;color:#7fd0ff;line-height:1;text-align:center}.nexus-ptime-knob-menu{position:absolute;right:0;bottom:52px;width:58px;display:none;gap:2px;padding:3px;border:1px solid rgba(151,193,232,.22);border-radius:9px;background:rgba(4,12,23,.96);box-shadow:0 12px 28px rgba(0,0,0,.35)}.nexus-ptime-knob-zone.open .nexus-ptime-knob-menu{display:grid}.nexus-ptime-knob-menu button{height:18px;border-radius:5px;border:1px solid rgba(151,193,232,.2);background:rgba(20,34,52,.72);color:#dceeff;font-size:6px;font-weight:950;letter-spacing:.08em}.nexus-ptime-knob-menu button.active{color:#fff;border-color:rgba(111,231,255,.6);background:rgba(22,58,88,.9)}
      ...
      @media(max-width:380px){#nexusProjectTimeOverlay{max-height:min(31dvh,242px);padding:6px 7px}.nexus-ptime-console{grid-template-columns:minmax(0,1fr) 52px;gap:5px}.nexus-ptime-left{grid-template-rows:15px 22px 15px}.nexus-ptime-ticker{height:15px}.nexus-ptime-ticker-text{font-size:5.9px;gap:12px;animation-duration:22s}.nexus-ptime-transport{height:22px;gap:2px}.nexus-ptime-transport button{height:22px!important}.nexus-ptime-transport button::before{font-size:10px;inset-bottom:4px}.nexus-ptime-knob-zone{height:54px}.nexus-ptime-knob{width:39px;height:39px}.nexus-ptime-mode-switch{height:15px!important}.nexus-ptime-mode-switch button{height:11px!important;line-height:11px!important;font-size:5.4px!important}.nexus-ptime-status-strip{height:17px}.nexus-ptime-status-item{font-size:5.8px}.nexus-ptime-status-item strong{font-size:8px}.nexus-ptime-event{min-height:27px}.nexus-ptime-event-list{max-height:86px}}
    `;
    ...
    root.innerHTML=`
      <div class="nexus-ptime-console">
        ...
        <div class="nexus-ptime-knob-zone" id="nexusPTimeKnobZone">
          ...
          <button class="nexus-ptime-knob" id="nexusPTimeKnob" type="button" aria-label="Open knob menu" aria-expanded="false"></button>
  ...
  function renderMarkers(){
    ...
    records.forEach(r=>{const s=document.createElement('span');s.className='nexus-ptime-marker'+(r.core?' core':'');s.style.left=(pctFor(r.date)*100)+'%';frag.appendChild(s)});
  ...
  function setEventListOpen(open){const root=$('nexusProjectTimeOverlay'),box=$('nexusPTimeEvent'),list=$('nexusPTimeEventList');if(!root||!box||!list)return;buildEventList();root.classList.toggle('events-open',open);box.setAttribute('aria-expanded',open?'true':'false');list.setAttribute('aria-hidden',open?'false':'true')}
  ...
  function setKnobMode(value){knobMode=value;const label=$('nexusPTimeKnobLabel');if(label)label.textContent=value;document.querySelectorAll('[data-knob]').forEach(btn=>btn.classList.toggle('active',btn.dataset.knob===value));const zone=$('nexusPTimeKnobZone'),knob=$('nexusPTimeKnob');if(zone)zone.classList.remove('open');if(knob)knob.setAttribute('aria-expanded','false')}

  function render(){
    const slider=$('nexusPTimeSlider');
    const p=slider?Number(slider.value)/10000:.72;
    const v=visible(p),latest=v[v.length-1],ph=phase(p),shownDate=mode==='simulation'?fmt(simAt(p)):fmt(srcAt(p));
    ...
    if($('nexusPTimeTicker'))$('nexusPTimeTicker').innerHTML=`<b>PROJECT TIME</b><strong>${shownDate}</strong><span>SOURCE ${fmt(srcAt(p))}</span><em>${compression.toFixed(1)}× COMPRESSED</em><span>${ph}</span><span>${v.length} DOCS · ${v.reduce((n,r)=>n+r.files.length,0)} FILES</span><span>${knobMode}</span>`;
    ...
    window.__NEXUS_PROJECT_TIME__={mode,progress:p,source:'project-time-knob-console'};
  }
  ...
  function bind(){
    ...
    $('nexusPTimeFfd')?.addEventListener('click',e=>{direction=1;step(900);flash(e.currentTarget)});
    $('nexusPTimeRwd')?.addEventListener('click',e=>{direction=-1;step(-900);flash(e.currentTarget)});
    ...
    $('nexusPTimeKnob')?.addEventListener('click',()=>{const zone=$('nexusPTimeKnobZone'),knob=$('nexusPTimeKnob');const open=!zone?.classList.contains('open');zone?.classList.toggle('open',open);knob?.setAttribute('aria-expanded',open?'true':'false')});
    document.querySelectorAll('[data-knob]').forEach(button=>button.addEventListener('click',()=>setKnobMode(button.dataset.knob)));
  }
  ...
  function tick(t){
    ...
    if(playing&&slider){
      ...
      const speed=knobMode==='TIME'?14:5;
      let next=Number(slider.value)+direction*dt*speed*34;
      if(next>=10000){next=10000;stopPlayback()}if(next<=0){next=0;stopPlayback()}
      slider.value=String(next);render();
    }
    last=t;requestAnimationFrame(tick);
  }

  function openPanel(){const p=panel();p.classList.add('open');p.setAttribute('aria-hidden','false');$('nexusTopTime')?.classList.add('active');if($('nexusTopTimeSub'))$('nexusTopTimeSub').textContent='100%';render()}
  function togglePanel(){const p=panel();const open=!p.classList.contains('open');p.classList.toggle('open',open);p.setAttribute('aria-hidden',open?'false':'true');$('nexusTopTime')?.classList.toggle('active',open);if($('nexusTopTimeSub'))$('nexusTopTimeSub').textContent=open?'100%':'LIVE'}
  function init(){
    injectStyle();
    const top=$('nexusTopTime');
    if(top&&!top.dataset.projectTimeKnobBound){top.dataset.projectTimeKnobBound='true';top.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();togglePanel()},{capture:true})}
